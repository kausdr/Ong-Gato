...
function Button({ children, order = 'primary', icon, isLoading = false, ...props }: ButtonProps) {
    ...
    return (
        <button
            {...props}
            className={`button inline-flex justify-center items-center gap-2 rounded py-2 px-4 font-sans transition-colors duration-200 ${buttonClass} ${props.className || ''} ${isDisabled ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}
            disabled={isDisabled}
        >
            {isLoading ? (
                <>
